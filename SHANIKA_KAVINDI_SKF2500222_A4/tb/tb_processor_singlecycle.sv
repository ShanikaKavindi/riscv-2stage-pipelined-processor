...
module processor_tb_singlecycle;
  parameter int WIDTH = 32;
  parameter string MEM_FILE = "program_singlecycle.hex";
  // Per A4 instructions, use 20ns clock period for single-cycle comparison.
  parameter int CLK_PERIOD = 20;          // ns (single-cycle clock)
  ...
  // DUT: single-cycle reference core
  processor_singlecycle #(.WIDTH(WIDTH)) dut (
    .clock(clock),
    .reset(reset),
    .memEn(memEn),
    .memData(memData),
  ...
  initial begin
    ...
    reset = 0;
    $display("[%0t] Released reset, single-cycle processor running.", $time);
  end
