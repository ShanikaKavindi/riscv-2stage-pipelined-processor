// Single-cycle RISC-V processor (reference implementation for A4 comparisons)
// Based on the provided Assignment 3 processor.sv, kept functionally identical.
//
// NOTE (per A4): keep MEM_DEPTH = 2048 for simulation and set MEM_DEPTH = 256 for synthesis.
//
// Supported (subset) RV32I + (MUL/DIV as in provided code):
// - R-type: add/sub/sll/slt/sltu/xor/srl/sra/or/and (+ mul/div by funct7[0])
// - I-type: addi/shift immediates
...
module processor_singlecycle #(
    ...
    logic isArithmetic, isImm, isMemRead, isLoadUI, isMemWrite, isBranch, isJAL, isJALR, isMUL, isAUIPC;
    logic isBranchR, isBranchC, regWriteEn;
    ...
    // Memory write port used only by the testbench "loader" interface
    always_ff @(posedge clock) begin
    ...
    always_comb begin
        ...
        isMUL        = (opcode == 5'b01100) & (funct7[0] == 1'b1); // MUL/DIV
        ...
        else if (isBranch)                   imm = WIDTH'($signed({ins[31], ins[7], ins[30:25], ins[11:8], 1'b0}));         // B-imm
    ...
    always_comb begin
        ...
        unique case (aluOp)
            ...
            DIV  : aluOut = ($signed(src2) == 0) ? 32'hFFFFFFFF :
                            (($signed(src1) == 32'h80000000 && $signed(src2) == -1) ? 32'h80000000 :
                              WIDTH'($signed(src1) / $signed(src2)));
            PASS : aluOut = src2;
            default: aluOut = 0;
        endcase
    end
    ...
    always_ff @(posedge clock) begin
        if (isMemWrite) begin
            case (funct3)
                3'b000: mainMemory[aluOut] <= memWrite[7:0];
                3'b001: begin
    ...
    always_comb begin
        case (funct3[1:0])
            2'b00: memRead = (!funct3[2]) ? ({{24{mainMemory[aluOut][7]}}, mainMemory[aluOut]}) :
                                          ({24'b0, mainMemory[aluOut]});
            2'b01: memRead = (!funct3[2]) ? ({{16{mainMemory[aluOut+1][7]}}, {mainMemory[aluOut+1], mainMemory[aluOut]}}) :
                                          ({16'b0, {mainMemory[aluOut+1], mainMemory[aluOut]}});
    ...
    always_ff @(posedge clock) begin
        if (regWriteEn && rd != 5'd0)
            registers[rd] <= regDataIn;
    end
